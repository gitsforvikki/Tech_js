...
function getUserDetails(userId) {
  return new Promise((resolve) => {
    setTimeout(() => {
      console.log("Fetched details for:", userId);
      resolve({ id: userId, name: "Vikash" });
    }, 1000);
  });
}
...
```javascript
    getUserId()
  .then((id) => {
    return getUserDetails(id); // returning next dependent promise
  })
  .then((user) => {
...
async function fetchUserData() {
  try {
    const id = await getUserId();        // wait for first promise
    const user = await getUserDetails(id); // depends on id
